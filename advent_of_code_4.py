...
my_input = """aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
def parse_room_data(room, replace=True):
    room = room.strip()
    letters, room_id, checksum = re.findall("^([a-z\-]+)\-([0-9]+)\[([a-z]{5})\]$", room)[0]
    if replace:
        letters = letters.replace("-", "")
    return letters, int(room_id), checksum

def validate_room_checksum(letters, room_id, checksum):
    counts = {letter: len(filter(lambda x: x == letter, letters)) for letter in letters}
    counts_list_alphabetically = sorted([(letter, counts[letter]) for letter in counts.keys()], key=lambda x: x[0])
    counts_list = sorted(counts_list_alphabetically, key=lambda x: x[1], reverse=True)
    actual_checksum = "".join([x[0] for x in counts_list[:5]])
    return actual_checksum == checksum

def find_valid(input_string):
    lines = input_string.split('\n')
    lines = [parse_room_data(line) for line in lines]
    valid_lines = filter(lambda line: validate_room_checksum(*line), lines)
    return valid_lines
    
def get_sum_sector_ids(valid_rooms):
    return sum([l[1] for l in valid_rooms])

def test():
    assert validate_room_checksum(*parse_room_data("aaaaa-bbb-z-y-x-123[abxyz]"))
    assert validate_room_checksum(*parse_room_data("a-b-c-d-e-f-g-h-987[abcde]"))
    assert validate_room_checksum(*parse_room_data("not-a-real-room-404[oarel]"))
    assert not validate_room_checksum(*parse_room_data("totally-real-room-200[decoy]"))
...
def test2():
    ...
    valid  = find_valid(lines)
    assert get_sum_sector_ids(valid) == 1514

test()
test2()

print(get_sum_sector_ids(find_valid(my_input)))

def test_shift():
    assert shift(parse_room_data("qzmt-zixmtkozy-ivhz-343[asdfg]", replace=False))[0] == "very encrypted name"
    
def shift(valid_room):
    to_shift = valid_room[0]
    shift_no = valid_room[1]
    shifted = []
    for letter in to_shift:
        if letter == '-':
            shifted.append(" ")
            continue
        new_letter = ord(letter) + shift_no % 26
        if new_letter > ord('z'):
            new_letter = ord('a') + (new_letter - ord('z') - 1)
        shifted.append(chr(new_letter))
    return "".join(shifted), shift_no

test_shift()

def north_pole_obejcts(input_string):
    lines = input_string.split('\n')
    lines = [parse_room_data(line, replace=False) for line in lines]
    lines = [shift(line) for line in lines]
    for line in lines:
        if "north" in line[0]:
            print(line)
    
north_pole_obejcts(my_input)
