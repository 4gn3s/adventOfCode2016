...
my_input = """xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
def abba_in(string):
    if len(string) < 4:
        return False
    for middle in range(2, len(string) - 1):
        if string[middle - 1] == string[middle] and \
               string[middle - 2] == string[middle + 1] and \
               string[middle] != string[middle + 1]:
               return True
    return False

def is_tls_supported(ip7_addr):
    words = re.findall("[\w']+", ip7_addr)
    brackets = [words[i] for i in range(len(words)) if i%2 != 0]
    non_brackets = [words[i] for i in range(len(words)) if i%2 == 0]
    brackets_with_abba = map(abba_in, brackets)
    non_brackets_with_abba = map(abba_in, non_brackets)
    if any(brackets_with_abba):
        return False
    return any(non_brackets_with_abba)
    
def find_aba(list_of_words):
    results = set()
    for word in list_of_words:
        for i, letter in enumerate(word[:-2]):
            if letter == word[i + 2]:
                results.add(letter + word[i + 1])
    return results
    
def is_ssl_supported(ip7_addr):
    words = re.findall("[\w']+", ip7_addr)
    brackets = [words[i] for i in range(len(words)) if i%2 != 0]
    non_brackets = [words[i] for i in range(len(words)) if i%2 == 0]
    all_aba = find_aba(non_brackets)
    all_bab = find_aba(brackets)
    for aba in all_aba:
        bab = aba[1] + aba[0]
        if bab in all_bab:
            return True
...
def test():
    ...
    assert not is_tls_supported("abcd[bddb]xyyx")
    assert not is_tls_supported("aaaa[qwer]tyui")
    assert is_tls_supported("ioxxoj[asdfgh]zxcvbn")

test()

def number_of_supporting(input_string, is_supported=is_tls_supported):
...
print(number_of_supporting(my_input))


def test_ssl():
    assert is_ssl_supported("aba[bab]xyz")
    assert not is_ssl_supported("xyx[xyx]xyx")
...
print(number_of_supporting(my_input, is_supported=is_ssl_supported))
