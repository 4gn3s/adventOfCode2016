...
my_input = """root@ebhq-gridcenter# df -h
...
def are_neighbours(node1ID, node2ID):
    x1, y1 = re.findall("/dev/grid/node-x(\d+)-y(\d+)", node1ID)[0]
    x2, y2 = re.findall("/dev/grid/node-x(\d+)-y(\d+)", node2ID)[0]
    neigh_1 = [(x1+1, y1), (x1-1, y1), (x1, y1+1), (x1, y1-1)]
    return (x2, xy) in neigh_1

def move(nodes, fromNID, toNID):
    if nodes[fromNID]["Used"] != 0:
        if are_neighbours(fromNID, toNID):
            if nodes[toNID]["Avail"] >= nodes[fromNID]["Used"]:
                to_move = nodes[fromNID]["Used"]
                # update size and use%
                nodes[fromNID]["Used"] = 0
                nodes[fromNID]["Avail"] += to_move
                nodes[toNID]["Used"] += to_move
                nodes[toNID]["Avail"] -= to_move

def is_viable_pair(nodes, fromNID, toNID):
    if fromNID != toNID:
        if nodes[fromNID]["Used"] != 0:
            if nodes[toNID]["Avail"] >= nodes[fromNID]["Used"]:
                return True
...
def find_viable_pairs_no(nodes):
    viable_pairs_counter = 0
    for node in nodes:
        for node2 in nodes:
            if is_viable_pair(nodes, node, node2):
                viable_pairs_counter += 1
    return viable_pairs_counter

def print_nodes(nodes):
    nodes_positions = []
    for node in nodes:
        x, y = re.findall("/dev/grid/node-x(\d+)-y(\d+)", node)[0]
        nodes_positions.append(((int(x), int(y)), nodes[node]["Used"], nodes[node]["Size"]))
    nodes_positions.sort(key=lambda ((x,y), used, avail): x)
    nodes_positions.sort(key=lambda ((x,y), used, avail): y)
    nodes_map = []
    row = []
    start = True
    ...
    for n in nodes_positions:
        if start or n[0][1] == row[-1][0][1]:
            row.append(n)
            start = False
        else:
            nodes_map.append(row)
            row = [n]
    nodes_map.append(row)
    nodes_map = [[pprint(node) for node in row] for row in nodes_map]
    nodes_map = ["".join(row) for row in nodes_map]
    nodes_map = "\n".join(nodes_map)
    print(nodes_map)


nodes = parse_nodes(my_input)
print_nodes(nodes)
print find_viable_pairs_no(nodes)
...
# print_nodes(parse_nodes(test_input))
